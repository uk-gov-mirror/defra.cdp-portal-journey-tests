...
describe('Services page', () => {
  ...
  describeWithAnnotations('When logged in as non-admin user', [], () => {
    ...
    describeWithAnnotations(
      'When viewing microservice as serviceOwner',
      [],
      () => {
...
describeWithAnnotations('Postgres service page', [], () => {
  const dbApplyChangelogEnv = 'dev'
  const postgresArtifactVersion = '0.100.0'

  describeWithAnnotations(
    'Logged in as "admin" with "restrictedTechPostgres" permission',
    [],
    () => {
      before(async () => {
      ...
      it('Should be redirected to the "Database update" deployment page', async () => {
        ...
        await $(
          `[data-testid="succeeded-status-tag"]*=Succeeded`
        ).waitForExist()
  ...
  describeWithAnnotations(
    'When logged in as non-admin user without "restrictedTechPostgres" permission',
    [],
    () => {
      before(async () => {
        await LoginStubPage.loginAsNonAdmin()
      ...
      describeWithAnnotations('When viewing a postgres service', [], () => {
        it("Should be able to navigate to a 'Service' page via the list", async () => {
          await ServicesPage.open()
